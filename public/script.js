...
var app = new Vue({
	...
	mounted: function() {
		console.log('hey hey, we are mounted');
	},
	created: function() {
	}
});
...
function init () {
	...
	var render = function() {
	    ...
	    TWEEN.update();
		console.log(camera);
		socket.emit('hello', camera.position);
	...
	function generateBoom (position, scale) {
		...
		var material = new THREE.MeshLambertMaterial( {color: 0xFFDDAA } );
	...
	function generateBullet () {
	    ...
	    cylinder[i].addEventListener('ready', function() {
			gunBang();
	        let self = cylinder[i];
			self.addEventListener( 'collision', function( other_object, relative_velocity, relative_rotation, contact_normal ) {
